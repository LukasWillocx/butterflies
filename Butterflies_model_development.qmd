...
library(stringr)
library(reticulate)
use_virtualenv("D:/.virtualenvs/r-tf-gpu", required = TRUE) #into the v_env
library(keras)
library(tensorflow)
```
...
```{r}
train_labels$path<- paste0("D:/data/butterflies/train/",train_labels$filename)
...
model <- keras_model_sequential() %>%
 layer_dense(units = 512, activation = "relu") %>%
  ...
  layer_flatten()%>%
  layer_dense(units = length(unique(train_generator$class_indices)), activation = 'softmax')
```
...
# Build the model using functional API (corrected approach)
...
history <- model %>% fit_generator(
  ...
  epochs = 100,
  validation_data = validation_generator,
  steps_per_epoch =  as.integer(train_generator$n / 64),
  validation_steps = as.integer(validation_generator$n / 64)
)
```
