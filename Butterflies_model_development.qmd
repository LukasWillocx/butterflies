...
# library(reticulate)
# use_virtualenv("D:/.virtualenvs/r-tf-gpu")
library(tensorflow)
```
...
Use the train generator (or validation generator for that matter) to construct a data-frame that identifies an index output (0-74) to a categorical butterfly species/class.
...
predict_image <- function(model,classes,image_path) {
  ...
  class_name<-classes$name[class_id]
  return(list(class = class_name, probability = max(pred)))
}
```
...
```{r}
predict_image(loaded_model,classes,'D:/data/butterflies/test/Image_400.jpg')
```
